...
def get_model(linear=True):
    varlist = mopeds.VariableList()
    varlist.add_variable(mopeds.VariableAlgebraic("y", 1))
    varlist.add_variable(mopeds.VariableControl("x", 1))
    varlist.add_variable(mopeds.VariableParameter("theta1", 1))
    varlist.add_variable(mopeds.VariableParameter("theta2", 2))

    model = mopeds.Model(varlist)
...
def generate_data(
    ...
    simulator = mopeds.SimulatorNLE(model, varlist)
...
if __name__ == "__main__":
    ...
    for bounds in [BOUNDS, BOUNDS2]:
        ...
        varlist_list = generate_varlist(LINEAR, x_data, y_data)
        pe = mopeds.ParameterEstimationNLE(model, varlist_list)
