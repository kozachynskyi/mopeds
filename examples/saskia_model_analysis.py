    ...
    def model_analysis(self):
        ...
        if not np.all(np.isnan(J0)):
            ...
            for equation_index, variable_index in index_nans:
                ...
                if (
                    previous_equation_index is None
                    or not previous_equation_index == equation_index
                ):
                    print(f"{equation_formulas[equation_index]}")
                previous_equation_index = equation_index

        nb, rowperm, colperm, rowblock, colblock, coarse_rowblock, coarse_colblock = (
            J0.sparsity().btf()
        )
        breakpoint()
        ...
        for block_index in range(0, nb):
            ...
            equation_indexes = rowperm[
                rowblock[block_index] : rowblock[block_index + 1]
            ]
            var_indexes = colperm[colblock[block_index] : colblock[block_index + 1]]
            block_equations = self.model.equations_algebraic.get(
                False, equation_indexes
            )
            ...
            for var_index in var_indexes:
                if (
                    variables_algebraic_list[var_index].casadi_var
                    == "e0_D_L_j1_ii3_jj1"
                ):
                    breakpoint()
            ...
            f = ca.rootfinder(
                "block_solver",
                "nlpsol",
                {"x": block_variables, "p": parameters_mx, "g": block_equations},
                solver_settings,
            )
