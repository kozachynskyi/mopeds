...
def test_artificial_data_generator_nle():
    ...
    res, par, _ = mopeds.tools.generate_artificial_data_from_grid_nle(
    ...
    res, par, _ = mopeds.tools.generate_artificial_data_from_grid_nle(
        ...
        measurement_names=["e0_F_s2"],
    )

    for varlist, expected in zip(
        res, [17.125730221093395, 18.367895136708697, 20.64042265044328]
    ):
...
def test_error_analyzer(scaling):
    ...
    with mopeds.options(variable_scaling=scaling):
        ...
        assert np.isclose(
            expected_predictions, np.array(analyzer.list_predictions)
        ).all()
